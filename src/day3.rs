...
fn find_o2(l1: &Vec<usize>, l2: &Vec<usize>, bit_pos: usize) -> Result<usize, String> {
    let &mut o2_list;
    ...
    // find the 2 lists and call this function again
    for val in o2_list {
        match val & (1 << bit_pos) {
    ...
    if bit_pos == 0 {
        ...
        if first_bit_0[0] & 1 != 0 {
            return Ok(first_bit_0[0]);
        } else {
            return Ok(first_bit_1[0]);
        }
    }

    find_o2(&first_bit_0, &first_bit_1, bit_pos - 1)
}
...
pub fn run(inputfile: &str) -> Result<(), String> {
    ...
    let o2 = find_o2(&first_bit_0, &first_bit_1, (VALUE_SIZE-2));

    println!("O2: {}", o2.unwrap());
